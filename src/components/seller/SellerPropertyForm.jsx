...
export default function SellerPropertyForm({ variant, propertyId, initialForm }) {
  ...
  const docsFolder = useRef(`docs/${Date.now()}`)
  const photosFolder = useRef(`photos/${Date.now()}`)
    ...
    <div className="min-h-screen px-4 py-12" style={{ background: bg }}>
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="text-white font-display text-2xl font-bold mt-3">{title}</h1>
          ...
          >
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={form.land_doc_type}
                    onChange={set('land_doc_type')}
                    className={inputCls}
                    placeholder={DOC_TYPE_BY_STATE[form.state] || 'e.g. Pahani / Adangal'}
                  />
                  {form.state && (
                    <p className="text-white/35 text-xs mt-1">
                      For {form.state}: {DOC_TYPE_BY_STATE[form.state]}
                    </p>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-3">
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    className="w-5 h-5 accent-turmeric-500"
                  />
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                />
                <FileUpload
                  key={`photos-${propertyId || 'new'}`}
                  bucket="property-photos"
                  folder={photosFolder.current}
                  accept="photos"
                  maxFiles={5}
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white/60 space-y-1">
                  ...
                  <p>
                    🌱 {form.land_soil_type} soil · {form.land_used_type}
                  </p>
                  <p>
                    📐 {form.area_acres} acres · ₹{Number(form.expected_price).toLocaleString('en-IN')}/acre
                  </p>
